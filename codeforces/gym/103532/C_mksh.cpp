...
struct tria {
    ...
    vector<vector<Real>> angs, sides;
    tria() {}
    tria(r a, r b, r c) : a(a), b(b), c(c) {
        side1 = sqrtl(sqdist(a, b));
        side2 = sqrtl(sqdist(b, c));
        side3 = sqrtl(sqdist(a, c));
        ang1 = getangl(a - c, b - c);
        ang2 = getangl(b - a, c - a);
        ang3 = getangl(a - b, c - b);
        ...
        // зеркальное
        // sides.push_back({side3, side2, side1});
        // angs.push_back({ang3, ang2, ang1});
        // sides.push_back({side1, side3, side2});
        // angs.push_back({ang1, ang3, ang2});
        // sides.push_back({side2, side1, side3});
        // angs.push_back({ang2, ang1, ang3});
    }
};
