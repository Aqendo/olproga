...
void replace(int l1, int r1, int l2, int r2) {
  auto [other, most_right] = split_sz(root, r2);
  auto [other2, second_arr] = split_sz(other, l2 - 1);
  auto [other3, middle] = split_sz(other2, r1);
  auto [most_left, first_arr] = split_sz(other3, l1 - 1);
  root = merge(most_left,
               merge(second_arr, merge(middle, merge(first_arr, most_right))));
}
