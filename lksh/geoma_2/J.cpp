...
void solve() {
    Point center, a, b;
    int radius;
    cin >> center >> radius >> a >> b;
    long double from_center_to_a = center.dist(a);
    long double from_center_to_b = center.dist(b);
    if (from_center_to_a <= radius && from_center_to_b >= radius ||
        from_center_to_a >= radius && from_center_to_b <= radius) {
        cout << "0\n";
        return;
    }
    long double from_point_to_segment_result =
        from_point_to_segment(center, a, b);
    if (from_center_to_a >= radius && from_center_to_b >= radius &&
        from_point_to_segment_result <= radius) {
        cout << "0\n";
        return;
    }
    if (from_point_to_segment_result >= radius) {
        cout << from_point_to_segment_result - radius << '\n';
        return;
    }
    cout << min({center.dist(a), center.dist(b), radius - center.dist(a),
                 radius - center.dist(b)})
         << '\n';
}
