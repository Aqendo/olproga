...
struct Hash {
    ...
    const bool operator==(const Hash& l) const {
        return size == l.size && sm == l.sm && xr == l.xr && hs == l.hs;
    }
};
Hash dp[6000][6000];
const int P = 2351;
void get(string& s) {
    int cur = root;
    for (int i = 0; i < (int)s.size(); ++i) {
        for (int starting_from = 0; starting_from <= i; ++starting_from) {
            if (i != starting_from) {
            ...
            dp[starting_from][i - starting_from].sm += change[el];
