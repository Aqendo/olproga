    ...
    for (int l = 1; l <= MAX_LOG; ++l) {
        for (int i = 0; i < (int)arr.size(); ++i) {
            ...
            if (i + (1ll << (l - 1)) < (int)arr.size() &&
                fi(dfs[l * sz_blocks + i]) >
                    fi(dfs[(l - 1) * sz_blocks + i + (1ll << (l - 1))])) {
                dfs[l * sz_blocks + i] = dfs[(l - 1) * sz_blocks + i + (1ll << (l - 1))];
            }
        }
    }
}
int64_t spget(int l, int r) {
    int LOG = __lg(r - l + 1);
    int64_t answer = dfs[LOG * sz_blocks + l];
    if (r - (1l << LOG) + 1 >= 0 && fi(answer) > fi(dfs[LOG * sz_blocks + r - (1l << LOG) + 1])) {
        // cout << getfi(tree[LOG][r - (1l << LOG) + 1]) << ' ' << getse(tree[LOG][r - (1l << LOG) + 1]) << endl;
        return dfs[LOG * sz_blocks + r - (1l << LOG) + 1];
    } else {
        return answer;
    }
}
...
int get(int l, int r) {
    ...
    for (; l < r && l % cnstnt != 0; ++l) {
        if (answer == INF || fi(answer) > fi(eulers[l])) {
            answer = eulers[l];
        }
    }
    for (; l < r && (r + 1) % cnstnt != 0; r--) {
        if (answer == INF || fi(answer) > fi(eulers[r])) {
            answer = eulers[r];
        }
    }
    if (l == r) {
    ...
    if (answer == INF || fi(answer) > fi(temp)) {
        answer = temp;
    }
    return se(answer);
}
void solve() {
    ...
    for (int i = 0; i < timer; ++i) {
        if (blocks[i / lg_] == INF || fi(blocks[i / lg_]) > fi(eulers[i])) {
            blocks[i / lg_] = eulers[i];
        }
    }
    spinit(blocks);
    answer += get(a1, a2);
