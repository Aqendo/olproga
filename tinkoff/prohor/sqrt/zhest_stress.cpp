...
#define uid(x, y) uniform_int_distribution<int>(x, y)(rng)
    ...
    while (true) {
        ...
        for (int i = 0; i < n; ++i) {
            int from = uid(1, n);
            queries[i] = {uid(1, 2), from, uid(from, n)};
        }
        answers.clear();
        vector<int> slow = get_slow();
        solve();
        if (answers != slow) {
            cout << n << ' ' << m << endl;
            for (int i = 0; i < n; ++i) {
