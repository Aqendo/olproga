...
struct Segtree {
    ...
    Node combine(const Node& l, const Node& r, int lazy_ = 0, Arifm arifm_ = {0, 0}, int lx = -69) const {
        // printf("l(%lld[%lld],ind=%lld], r(%lld[%lld],ind=%lld], lazy=%lld, arifm_=(%lld;%lld)\n", l.sum, l.len, l.index, r.sum, r.len, r.index, lazy_,
        // arifm_.a1, arifm_.k);
    ...
    void add(int x, int lx, int rx, int l, int r, int a1, int k) {
        ...
        if (l == lx && r == rx) {
            ...
            if (lx != rx) {
                ...
                // printf("combine add2\n");
                tree[x] = combine({0, 1, lx}, {0, 0}, lazy_add[x], lazy_arifm[x]);
            }
            return;
        }
        int m = (lx + rx) >> 1;
        // printf("add_add1 x=%lld, lx=%lld, rx=%lld, l=%lld, r=%lld\n", x << 1, lx, m, l, max(m + 1, l));
